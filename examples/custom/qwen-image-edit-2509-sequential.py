...
# Load conditioning images once and reuse for all batch items
base_image1 = load_image("https://huggingface.co/datasets/nunchaku-tech/test-data/resolve/main/inputs/man.png").convert("RGB")
base_image2 = load_image("https://huggingface.co/datasets/nunchaku-tech/test-data/resolve/main/inputs/puppy.png").convert("RGB")
base_image3 = load_image("https://huggingface.co/datasets/nunchaku-tech/test-data/resolve/main/inputs/sofa.png").convert("RGB")
...
# Process prompts one at a time (batching multiple prompts in one call may not be supported)
...
while time.time() - start_time < duration_seconds:
    # Loop through prompts cyclically
    prompt = prompts[execution_count % len(prompts)]

    inputs = {
        "image": [base_image1, base_image2, base_image3],
        "prompt": prompt,
        "true_cfg_scale": 4.0,
        "negative_prompt": " ",
...
for idx, image in enumerate(all_images):
    image.save(f"qwen-image-edit-2509-r{rank}_sequential_{timestamp}_{idx}.png")
